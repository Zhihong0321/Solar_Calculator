...
try {
  ({ resolveAgentAuditContext } = require('./agentAuditContext'));
} catch (err) {
  if (err?.code !== 'MODULE_NOT_FOUND') {
    throw err;
  }
  console.warn('[InvoiceService] agentAuditContext unavailable, using basic audit fallback.');
}
...
async function createInvoice(pool, invoiceRequestPayload) {
  ...
  try {
    ...
    const repoPayload = {
      ...
      discountDescription: invoiceRequestPayload.discount_description || null,
